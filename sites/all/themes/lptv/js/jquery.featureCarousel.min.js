...
function setupFeatureDimensions(){pluginData.containerWidth=pluginData.featuresContainer.width();pluginData.containerHeight=pluginData.featuresContainer.height();var $firstFeatureImage=$(pluginData.containerIDTag).find(".carousel-feature img:first");if(options.largeFeatureWidth>1)
...
pluginData.featuresContainer.find(".carousel-feature").each(function(){$(this).css({'left':(pluginData.containerWidth/2)-(pluginData.smallFeatureWidth/2)-(pluginData.borderWidth/2),'width':pluginData.smallFeatureWidth,'height':pluginData.smallFeatureHeight,'top':options.smallFeatureOffset+options.topPadding,'opacity':0});}).find(".carousel-feature img").css({'width':pluginData.smallFeatureWidth});if(options.captionBelow){pluginData.featuresContainer.find('.carousel-caption').css('position','relative');}
...
move(direction);}
setTimer(false);}).find('.carousel-feature img').animate({width:new_width,height:new_height},options.carouselSpeed,options.animationEasing).end();}
...
event.preventDefault();return false;}else{options.clickedCenter($(this));}}});});$(".tracker-individual-blip").live("click",function(){var goTo=$(this).attr("id").substring(8);var whereIsIt=pluginData.featuresContainer.find(".carousel-feature").eq(goTo-1).data('position');var currentlyAt=pluginData.currentCenterNum;if(goTo!=currentlyAt){var shortest=findShortestDistance(1,whereIsIt);if(shortest<0){initiateMove(false,(shortest*-1));}else{initiateMove(true,shortest);}}});});};$.fn.featureCarousel.defaults={largeFeatureWidth:190,largeFeatureHeight:270,smallFeatureWidth:200*0.85,smallFeatureHeight:270*0.85,topPadding:0,sidePadding:80,smallFeatureOffset:10,startingFeature:1,carouselSpeed:1000,autoPlay:0,pauseOnHover:true,stopOnHover:false,preload:true,displayCutoff:0,animationEasing:'swing',captionBelow:true,movedToCenter:$.noop,leavingCenter:$.noop,clickedCenter:$.noop};})(jQuery);
